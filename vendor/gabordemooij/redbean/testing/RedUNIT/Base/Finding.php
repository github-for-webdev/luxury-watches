...
class Finding extends Base
{
	...
	public function testPreloadUsingFindMulti()
	{
		...
		$all = R::findMulti(
			'country',
			'SELECT country.* FROM country WHERE id IN (' . R::genSlots($gebruikers) . ') ',
			colfield($gebruikers, 'country_id'),
			array(
				array(
					'a' => 'country',
					'b' => $gebruikers,
		...
		$all = R::findMulti(
			'country',
			R::genSlots($gebruikers, 'SELECT country.* FROM country WHERE id IN ( %s )'),
			colfield($gebruikers, 'country_id'),
			array(
				Finder::onMap('country', $gebruikers)
			)
	...
	public function testExplParaBin()
	{
		...
		$found = R::findOne(
			'bean',
			' property = ? AND property2 = ? AND property3 = ? ',
	...
	public function testMultiAdvanced()
	{
		...
		$collection = R::findMulti(
			'book,page,text,category,book_category',
			'
